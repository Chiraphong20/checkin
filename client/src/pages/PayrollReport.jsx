...
const PayrollReport = () => {
  ...
  const calculateDailyStatus = (employee, dateStr, checkinRecord, leaveRecord, branchConfigsMap) => {
      ...
      // 2.0.1 Office (02) -> เช็ควันหยุด
      if (employee.department === '02') {
          const currentDay = dayjs(dateStr);
          const isWeekend = currentDay.day() === 0 || currentDay.day() === 6; 
          
          const empBranchConfig = branchConfigsMap[employee.branch];
          const empBranchId = empBranchConfig ? empBranchConfig.id : null;

          const isPublicHoliday = holidays.some(h => 
              h.date === dateStr && 
              (!h.targetBranches || h.targetBranches === "ALL" || h.targetBranches.length === 0 || (Array.isArray(h.targetBranches) && empBranchId && h.targetBranches.includes(empBranchId)))
          );
      ...
      // 2.3 มี Checkin -> ใช้ข้อมูลจริงจาก DB ถ้ามี
      if (checkinRecord.fine !== undefined && checkinRecord.status) {
         let finalFine = checkinRecord.fine;
         if (checkinRecord.status.includes("ปกติ")) finalFine = 0;
      ...
      // Fallback calculation (กรณีคำนวณใหม่)
      const branchName = checkinRecord.branch;
      const config = branchConfigsMap[branchName] || {}; 
      
      // 🔥 แก้ไข: แปลง shift เป็น Number เพื่อความชัวร์
      const shift = Number(checkinRecord.shift) || 1; 

      const isShift2 = shift === 2;
      // ถ้าเป็นกะ 2 ใช้ shift2_, ถ้าไม่ใช้ default
      const prefix = isShift2 ? 'shift2_' : (config.shift1_startTime ? 'shift1_' : '');
  ...
  const handleCalculateReport = () => {
    ...
    const report = employees.map(emp => {
        ...
        const details = [];
        const allDailyRecords = []; // ✅ เก็บข้อมูลดิบรายวัน
        ...
        while (curr.isSameOrBefore(last)) {
            ...
            allDailyRecords.push({
                ...
                checkinTime: dailyResult.checkinTime,
                status: dailyResult.status,
                fine: dailyResult.fine
            });
        ...
        return {
            ...
            workDays, lateDays, absentDays, leaveDays, 
            details,
            allDailyRecords // ✅ แนบข้อมูลรายวัน
        };
    });
  ...
  const exportMainExcel = () => {
    ...
    XLSX.writeFile(wb, `Payroll_Summary_${dayjs().format('YYYYMMDD')}.xlsx`);
  };

  // ✅ Export รายวันละเอียด (Daily Detail)
  const exportDailyExcel = () => {
      // ดึง allDailyRecords ของทุกคนที่ Filter อยู่มาต่อกัน
      const allRows = filteredData.flatMap(emp => emp.allDailyRecords);
      ...
      const ws = XLSX.utils.json_to_sheet(dataToExport);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "Daily_Detail");
      XLSX.writeFile(wb, `Payroll_Daily_${dayjs().format('YYYYMMDD')}.xlsx`);
  };
    ...
    <div style={{ padding: 20 }}>
      ...
      <Spin spinning={calculating || loading}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
          ...
          />
          <Space>
             <Button icon={<FileExcelOutlined />} onClick={exportMainExcel} disabled={filteredData.length === 0} style={{ backgroundColor: '#1D6F42', color: 'white' }}>
                 Export สรุป (Summary)
             </Button>
             {/* ✅ ปุ่มใหม่: Export รายวัน */}
             <Button icon={<FileExcelOutlined />} onClick={exportDailyExcel} disabled={filteredData.length === 0} style={{ backgroundColor: '#1890ff', color: 'white' }}>
                 Export รายวัน (Daily)
             </Button>
          </Space>
        </div>
