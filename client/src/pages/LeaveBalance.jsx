...
export default function LeaveBalance() {
  ...
  // ✅ เพิ่ม State เก็บ ID ของสาขาพนักงาน
  ...
  // ✅ แก้ไข Logic การเช็คสาขา (ใช้ ID เทียบ)
  const calculateCompensatory = (allHolidays, allLeaves, branchId, isSales) => {
      const earned = allHolidays.filter(h => {
          ...
          if (h.targetBranches && Array.isArray(h.targetBranches)) {
              // ✅ เทียบ ID กับ ID แล้ว (ถูกต้อง)
              isBranchOff = h.targetBranches.includes(branchId);
          ...
          // ได้ชดเชยถ้า: วันผ่านไปแล้ว และ สาขาเราไม่ได้หยุด
  ...
  useEffect(() => {
    const initLiff = async () => {
      try {
        ...
        // ✅ 1.5 ดึงข้อมูลสาขา เพื่อแปลง "ชื่อสาขา" เป็น "ID สาขา"
        let currentBranchId = empData.branch; // ค่าเริ่มต้น (เผื่อเก็บเป็น ID อยู่แล้ว)
        try {
            const branchesSnap = await getDocs(collection(db, "branches"));
            const foundBranch = branchesSnap.docs.find(d => d.data().name === empData.branch);
            if (foundBranch) {
                currentBranchId = foundBranch.id; // ได้ ID ที่ถูกต้อง: "3hLUNUaRI3TLYZZhlYGM"
            }
        } catch (e) { console.error("Error fetching branches", e); }
        
        setMyBranchId(currentBranchId); // เก็บไว้ใช้ตอน render
        ...
        // ✅ กรองวันหยุดเฉพาะของสาขาเรา (เพื่อเอาไปนับวันหยุดในเดือน/ปี)
        const myHolidays = holidaysData.filter(h => {
            let isOff = true;
            if (h.targetBranches && Array.isArray(h.targetBranches)) {
                isOff = h.targetBranches.includes(currentBranchId); // ✅ เทียบด้วย ID
            } else if (h.targetBranches !== "ALL" && h.targetBranches) {
                isOff = false; 
            }
            // เงื่อนไข Sales
            if (empData.department === "03" && !h.allowSales) isOff = false;
            return isOff;
        });

        const publicHolidaysDates = myHolidays.map(h => h.date); // วันที่ที่เราได้หยุดจริง
        ...
        const holidaysInYear = publicHolidaysDates.filter(date => date.startsWith(currentYear)).length;
        const holidaysInMonth = publicHolidaysDates.filter(date => date.startsWith(currentMonthStr)).length;
        ...
        if (isOffice) {
            ...
            usedMonth = allRecords.filter(r => {
                ...
                const isHoliday = publicHolidaysDates.includes(r.date);
            ...
            usedMonth = allRecords.filter(r => {
                const isThisMonth = r.date.startsWith(currentMonthStr);
                const isHoliday = publicHolidaysDates.includes(r.date); // ✅ เช็คกับวันหยุดของสาขาเราจริงๆ
                const isVacation = r.status.includes("พักร้อน");
        ...
        // ✅ ส่ง ID สาขาที่ถูกต้องไปคำนวณ
        const compData = calculateCompensatory(holidaysData, leaves, currentBranchId, isSales);
        ...
        setLeaveData({
            ...
            isSalesOrTransport: isSalesOrTransport,
  ...
  // ✅ ใช้ allPublicHolidays ในการกรองแสดงผล (เพราะใน Modal อยากดูวันหยุดทั้งหมด)
    ...
    <div style={{ minHeight: "100vh", background: "#f5f7fa", paddingBottom: 40, fontFamily: "'Sarabun', sans-serif" }}>
      ...
      >
        ...
        <List
            ...
            renderItem={(item) => {
                // ใช้ myBranchId ที่หามาได้จากชื่อสาขา
                let isMyBranchOff = true;
                if (item.targetBranches && Array.isArray(item.targetBranches)) {
                    isMyBranchOff = item.targetBranches.includes(myBranchId); // ✅ เทียบ ID กับ ID
