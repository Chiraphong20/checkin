  ...
  useEffect(() => {
    const initLiff = async () => {
      try {
        ...
        // 2. ดึงข้อมูลวันหยุดนักขัตฤกษ์ (ทั้ง Object เพื่อเอาชื่อวันหยุด)
        ...
        const publicHolidayDates = publicHolidaysData.map(h => h.date);  // เก็บเฉพาะวันที่
        ...
        // 🔥 เพิ่มวันหยุดนักขัตฤกษ์ลงในประวัติ (เฉพาะ Office/Admin)
        if (isOffice) {
            publicHolidaysData.forEach(h => {
                if (h.date.startsWith(currentYear)) {
                    // ถ้าวันนั้นไม่ได้มีการบันทึกอื่น (เช่น มาทำงาน หรือ ลาป่วย) ให้ใส่วันหยุดนักขัตฤกษ์ลงไป
        ...
        let annualTotal = 0;
        let annualUsed = 0;
        if (yearsOfService >= 1) annualTotal = isOffice ? 6 : 11;
        ...
        if (isOffice) {
            // Office: โควตา = เสาร์อาทิตย์
            monthlyQuota = countWeekends(dayjs());
            // นับวันที่ใช้ (ไม่รวมนักขัตฤกษ์ ไม่รวมพักร้อน)
            usedMonth = allRecords.filter(r => {
                const isThisMonth = r.date.startsWith(currentMonthStr);
                const isHoliday = r.type === "holiday"; // เช็คจาก type ที่เราเพิ่งใส่ไป
        ...
        // เรียงลำดับวันที่ (เฉพาะเดือนนี้)
        const history = allRecords
            .filter(r => r.date.startsWith(currentMonthStr))
            .sort((a,b) => dayjs(b.date).diff(dayjs(a.date))); // เรียงใหม่ไปเก่า
        ...
        <List
            ...
            renderItem={(item) => {
                ...
                return (
                  <List.Item>
                    <List.Item.Meta
                      avatar={<Avatar style={{ backgroundColor: '#fff', color: info.color, border: `1px solid ${info.color}` }} icon={info.icon} />}
