...
export default function LeaveBalance() {
  ...
  // ✅ แก้ไข Logic คำนวณชดเชย (Map ชื่อสาขา -> ID)
  const calculateCompensatory = (allHolidays, allLeaves, branchId, deptCode) => {
      ...
      const earned = allHolidays.filter(h => {
          ...
          let isMyBranchStopped = false;
          
          if (!h.targetBranches || h.targetBranches === "ALL" || h.targetBranches.length === 0) {
              isMyBranchStopped = true; 
          } else if (Array.isArray(h.targetBranches)) {
              isMyBranchStopped = h.targetBranches.includes(branchId); 
          }

          if (isMyBranchStopped && !h.allowSales) {
              isMyBranchStopped = false; 
          }

          return isPast && !isMyBranchStopped;
      }).length;
    ...
    const initLiff = async () => {
      try {
        ...
        branchesSnap.forEach(doc => {
            const data = doc.data();
            bMap[data.name] = doc.id; // Map ชื่อ -> ID
        });
        ...
        // Filter วันที่พนักงานคนนี้หยุดจริง (เพื่อคำนวณโควต้า)
        const myHolidaysDates = holidaysData.filter(h => {
            if (["01", "02"].includes(empData.department)) return true; // Office หยุด
            
            let isBranchOff = false;
            if (!h.targetBranches || h.targetBranches === "ALL") isBranchOff = true;
        ...
        setLeaveData({
            ...
            annualLeaveTotal, annualLeaveUsed: annualUsedTotal,
            usedLeaveMonth: usedMonth,
            holidaysInMonth, holidaysInYear,
            ...
            <Card bordered={false} style={{ borderRadius: 16, marginBottom: 20, background: '#f9f0ff', border: '1px solid #d3adf7' }}>
                ...
                <div style={{ marginTop: 10 }}>
                    ...
                    <Text type="secondary" style={{ fontSize: 10, display: 'block', marginTop: 4 }}>
                        *ได้รับเมื่อวันหยุดผ่านไปแล้ว และร้านไม่ได้หยุด
                    </Text>
                </div>
            </Card>
        )}
